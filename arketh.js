...
function eval_arketh(s) {
    if(s.length > 1) {
        F = eval_arketh(s.substr(0, s.length - 1));
...
function eval_arketh_cps(s, k) {
    if(s.length > 1) {
        return eval_arketh_cps(
...
function eval_arketh_cpst(s, k) {
    if(s.length > 1) {
        return function () {
            return eval_arketh_cpst(
                s.substr(0, s.length - 1),
                function(F) {
                    if(s[s.length - 1] == '0') {
                        return function () {
...
function drive_cpst(p, limit) {
    var count = 0;
    var done = false;
    var thunk = eval_arketh_cpst(p, function (x) {
        //alert("done!");
...
var ARKeth_active = true;

var toggle_pause = function () { };

function startup() { 
    var c = document.getElementById('#ARKeth');
    ...
    function drawPoints() {
        for(var i = 0; i < 100; i++) {
            ...
            var pos = d2xy(encode_arketh(prog));
